...
def calc_returns(payoffs, agents, rollouts):
    ...
    for idx, payoff_hist in enumerate(payoffs):
        payoff_sum = np.sum(payoff_hist, axis=0)  #？这里是什么意思
...
def reset(num_agents, num_actions, num_objectives, alpha_lq, alpha_ltheta, alpha_fq, alpha_ftheta, alpha_mq,
    ...
    for ag in range(num_agents):
        ...
        new_agent = CoopActionAgent(ag, u, du, alpha_lq, alpha_ltheta, alpha_decay, num_actions, num_objectives,opt)
...
def run_experiment(experiment, runs, episodes, rollouts, payoff_matrix, opt_init, seed=1):
    ...
    for run in range(runs):      #实验运行循环 每次实验100次   100 * 5000
        ...
        agents = reset(num_agents, num_actions, num_objectives, alpha_lq, alpha_ltheta, alpha_fq,
        ...
        for episode in range(episodes):   #集数循环 每集5000次迭代
            ...
            communicator = episode % len(agents)   #选择通信代理  我改的时候领导者固定
            ...
            for rollout in range(rollouts):  # Required to evaluate the SER and action probabilities.
                message = communicating_agent.get_message()    #滚动期循环    7-15行
                actions = select_actions(agents, message)
                payoffs = calc_payoffs(agents, actions, payoff_matrix)  #改成目标函数，写一个大函数 
                                                            #？payoff_matrix为什么这么多东西
            ...
            last_actions = np.array(ep_actions)[:, -1]    #更新代理   16-22
            last_payoffs = np.array(ep_payoffs)[:, -1]
            last_message = ep_messages[-1]
            update(agents, communicator, last_message, last_actions, last_payoffs)  # Update the agents. 23-28

            # Get the necessary results from this episode.  #计算结果
            action_probs = calc_action_probs(ep_actions, num_actions, rollouts)  #每个 roollout 统计一次
...
#解析命令行参数
    parser.add_argument('--game', type=str, default='game2', help="which MONFG game to play")
